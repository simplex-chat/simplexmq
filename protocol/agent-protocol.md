...
All parameters are present in the URI in hash part so that they are not sent to the Internet (in case "https" scheme is used) and can only be used by the client-side code and processed by the client application. Parameters `smp` and `e2e` can be present in any order, any unknown additional parameters SHOULD be ignored.
