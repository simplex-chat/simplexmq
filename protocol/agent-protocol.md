...
newCmd = %s"NEW" SP <smpServer> [SP %s"NO_ACK"] ; `smpServer` is the same as in out-of-band message
...
<!-- TODO align out-of-band message abnf in SMP protocol and SMP agent protocol (different names for same things) -->
```abnf
