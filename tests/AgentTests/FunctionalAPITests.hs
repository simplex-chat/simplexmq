...
pGet c = do
  ...
  case cmd of
    CONNECT {} -> liftIO (putStrLn $ show (clientId c) <> " CONNECT") >> pGet c
    DISCONNECT {} -> liftIO (putStrLn $ show (clientId c) <> " DISCONNECT") >> pGet c
    _ -> pure t
...
getSMPAgentClient' clientId cfg' initServers dbPath = do
  ...
  c <- getSMPAgentClient_ (Just clientId) cfg' initServers st False
