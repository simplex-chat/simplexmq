...
createConnection c userId enableNtfs cMode clientData subMode = do
  (connId, (_, CCLink cReq _, Nothing)) <- A.createConnection c NRMInteractive userId Nothing enableNtfs True cMode Nothing clientData IKPQOn subMode
  pure (connId, cReq)
...
runAgentClientTestPQ sqSecured viaProxy (alice, aPQ) (bob, bPQ) baseId =
  runRight_ $ do
    (bobId, (_, CCLink qInfo Nothing, Nothing)) <- A.createConnection alice NRMInteractive 1 Nothing True True SCMInvitation Nothing Nothing aPQ SMSubscribe
...
runAgentClientContactTestPQ sqSecured viaProxy reqPQSupport (alice, aPQ) (bob, bPQ) baseId =
  runRight_ $ do
    (_, (_, CCLink qInfo Nothing, Nothing)) <- A.createConnection alice NRMInteractive 1 Nothing True True SCMContact Nothing Nothing aPQ SMSubscribe
    aliceId <- A.prepareConnectionToJoin bob 1 True qInfo bPQ
...
testRejectContactRequest =
  withAgentClients2 $ \alice bob -> runRight_ $ do
    (_addrConnId, (_, CCLink qInfo Nothing, Nothing)) <- A.createConnection alice NRMInteractive 1 Nothing True True SCMContact Nothing Nothing IKPQOn SMSubscribe
...
testInvitationShortLink viaProxy a b =
  withAgent 3 agentCfg initAgentServers testDB3 $ \c -> do
    ...
    (bId, (_, CCLink connReq (Just shortLink), Nothing)) <- runRight $ A.createConnection a NRMInteractive 1 Nothing True True SCMInvitation (Just newLinkData) Nothing CR.IKUsePQ SMSubscribe
...
testInvitationShortLinkPrev viaProxy sndSecure a b = runRight_ $ do
  ...
  (bId, (_, CCLink connReq Nothing, Nothing)) <- A.createConnection a NRMInteractive 1 Nothing True True SCMInvitation (Just newLinkData) Nothing CR.IKPQOn SMSubscribe
...
testContactShortLink viaProxy a b =
  withAgent 3 agentCfg initAgentServers testDB3 $ \c -> do
    ...
    (contactId, (_, CCLink connReq0 (Just shortLink), Nothing)) <- runRight $ A.createConnection a NRMInteractive 1 Nothing True True SCMContact (Just newLinkData) Nothing CR.IKPQOn SMSubscribe
...
testAddContactShortLink viaProxy a b =
  withAgent 3 agentCfg initAgentServers testDB3 $ \c -> do
    (contactId, (_, CCLink connReq0 Nothing, Nothing)) <- runRight $ A.createConnection a NRMInteractive 1 Nothing True True SCMContact Nothing Nothing CR.IKPQOn SMSubscribe
    Right connReq <- pure $ smpDecode (smpEncode connReq0) --
    let userData = UserLinkData "some user data"
        userCtData = UserContactData {direct = True, owners = [], relays = [], userData}
        newLinkData = UserContactLinkData userCtData
...
testInvitationShortLinkRestart ps = withAgentClients2 $ \a b -> do
  let userData = UserLinkData "some user data"
      newLinkData = UserInvLinkData userData
  (bId, (_, CCLink connReq (Just shortLink), Nothing)) <- withSmpServer ps $
    runRight $ A.createConnection a NRMInteractive 1 Nothing True True SCMInvitation (Just newLinkData) Nothing CR.IKUsePQ SMOnlyCreate
  withSmpServer ps $ do
...
testContactShortLinkRestart ps = withAgentClients2 $ \a b -> do
  let userData = UserLinkData "some user data"
      userCtData = UserContactData {direct = True, owners = [], relays = [], userData}
      newLinkData = UserContactLinkData userCtData
  (contactId, (_, CCLink connReq0 (Just shortLink), Nothing)) <- withSmpServer ps $
    runRight $ A.createConnection a NRMInteractive 1 Nothing True True SCMContact (Just newLinkData) Nothing CR.IKPQOn SMOnlyCreate
  Right connReq <- pure $ smpDecode (smpEncode connReq0)
  let updatedData = UserLinkData "updated user data"
      updatedCtData = UserContactData {direct = False, owners = [], relays = [relayLink1, relayLink2], userData = updatedData}
...
testAddContactShortLinkRestart ps = withAgentClients2 $ \a b -> do
  let userData = UserLinkData "some user data"
      userCtData = UserContactData {direct = True, owners = [], relays = [], userData}
      newLinkData = UserContactLinkData userCtData
  ((contactId, (_, CCLink connReq0 Nothing, Nothing)), shortLink) <- withSmpServer ps $ runRight $ do
    r@(contactId, _) <- A.createConnection a NRMInteractive 1 Nothing True True SCMContact Nothing Nothing CR.IKPQOn SMOnlyCreate
...
testOldContactQueueShortLink ps@(_, msType) = withAgentClients2 $ \a b -> do
  (contactId, (_, CCLink connReq Nothing, Nothing)) <- withSmpServer ps $ runRight $
    A.createConnection a NRMInteractive 1 Nothing True True SCMContact Nothing Nothing CR.IKPQOn SMOnlyCreate
...
makeConnectionForUsers_ pqSupport sqSecured alice aliceUserId bob bobUserId = do
  (bobId, (_, CCLink qInfo Nothing, Nothing)) <- A.createConnection alice NRMInteractive aliceUserId Nothing True True SCMInvitation Nothing Nothing (IKLinkPQ pqSupport) SMSubscribe
...
testSetConnShortLinkAsync ps = withAgentClients2 $ \alice bob ->
  withSmpServerStoreLogOn ps testPort $ \_ -> runRight_ $ do
    let userData = UserLinkData "test user data"
        userCtData = UserContactData {direct = True, owners = [], relays = [], userData}
        newLinkData = UserContactLinkData userCtData
    (cId, (_, CCLink qInfo (Just shortLink), _)) <- A.createConnection alice NRMInteractive 1 Nothing True True SCMContact (Just newLinkData) Nothing IKPQOn SMSubscribe
...
testGetConnShortLinkAsync ps = withAgentClients2 $ \alice bob ->
  withSmpServerStoreLogOn ps testPort $ \_ -> runRight_ $ do
    let userData = UserLinkData "test user data"
        userCtData = UserContactData {direct = True, owners = [], relays = [], userData}
        newLinkData = UserContactLinkData userCtData
    (_, (_, CCLink qInfo (Just shortLink), _)) <- A.createConnection alice NRMInteractive 1 Nothing True True SCMContact (Just newLinkData) Nothing IKPQOn SMSubscribe
...
testClientNotice ps = do
  withAgent 1 agentCfg initAgentServers testDB $ \c -> do
    (cId, _) <- withSmpServerStoreLogOn ps testPort $ \_ -> runRight $
      A.createConnection c NRMInteractive 1 Nothing True True SCMContact Nothing Nothing IKPQOn SMSubscribe
    ("", "", DOWN _ [_]) <- nGet c
    ...
    (cId', _) <- withSmpServerStoreLogOn ps testPort $ \_ -> do
      ...
      testNotice c True
      threadDelay 1000000
      runRight $ A.createConnection c NRMInteractive 1 Nothing True True SCMContact Nothing Nothing IKPQOn SMSubscribe
    ("", "", DOWN _ [_]) <- nGet c
  ...
  withAgent 1 agentCfg initAgentServers testDB $ \c -> do
    withSmpServerStoreLogOn ps testPort $ \_ -> do
      ...
      void $ runRight $ A.createConnection c NRMInteractive 1 Nothing True True SCMContact Nothing Nothing IKPQOn SMSubscribe
  where
    ...
    testNotice c willExpire = do
      NOTICE "localhost" False expiresAt_ <- runLeft $ A.createConnection c NRMInteractive 1 Nothing True True SCMContact Nothing Nothing IKPQOn SMSubscribe
