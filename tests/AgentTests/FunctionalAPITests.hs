...
import Simplex.Messaging.Agent.Client (AgentQueueInfo (..), ProtocolTestFailure (..), ProtocolTestStep (..), UserNetworkInfo (..), UserNetworkType (..), waitForUserNetwork)
...
testServerQueueInfo = do
  ...
  where
    ...
    checkQ c cId qiSnd' qiSubThread_ qiSize' msgType_ = do
      AgentQueueInfo {info = QueueInfo {qiSnd, qiNtf, qiSub, qiSize, qiMsg}} <- getConnectionQueueInfo c cId
      liftIO $ do
