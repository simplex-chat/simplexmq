...
testAgentClient = do
  ...
  Right () <- runExceptT $ do
    (bobId, qInfo) <- createConnection alice CMInvitation
    aliceId <- joinConnection bob qInfo "bob's connInfo"
    ("", _, REQ CMInvitation confId "bob's connInfo") <- get alice
    acceptConnection alice bobId CMInvitation confId "alice's connInfo"
    get alice ##> ("", bobId, CON)
