...
testConcurrentWrites =
  it "should complete multiple concurrent write transactions w/t sqlite busy errors" $ \(s1, s2) -> do
    g <- newTVarIO =<< drgNew
    _ <- runExceptT $ createRcvConn s1 g cData1 rcvQueue1 CMInvitation
...
testCreateRcvConn =
  it "should create RcvConnection and add SndQueue" $ \store -> do
    g <- newTVarIO =<< drgNew
    createRcvConn store g cData1 rcvQueue1 CMInvitation
      `returnsResult` "conn1"
    getConn store "conn1"
...
testCreateRcvConnRandomId =
  it "should create RcvConnection and add SndQueue with random ID" $ \store -> do
    g <- newTVarIO =<< drgNew
    Right connId <- runExceptT $ createRcvConn store g cData1 {connId = ""} rcvQueue1 CMInvitation
...
testCreateRcvConnDuplicate =
  it "should throw error on attempt to create duplicate RcvConnection" $ \store -> do
    g <- newTVarIO =<< drgNew
    _ <- runExceptT $ createRcvConn store g cData1 rcvQueue1 CMInvitation
    createRcvConn store g cData1 rcvQueue1 CMInvitation
...
testCreateRcvMsg =
  it "should reserve internal ids and create a RcvMsg" $ \st -> do
    g <- newTVarIO =<< drgNew
    let ConnData {connId} = cData1
    _ <- runExceptT $ createRcvConn st g cData1 rcvQueue1 CMInvitation
