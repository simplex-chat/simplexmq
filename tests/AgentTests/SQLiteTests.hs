...
testCompiledThreadsafe = do
  it "should throw error if compiled sqlite library is not threadsafe" $ \store -> do
    ...
    -- let threadsafeOption = find (isPrefixOf "THREADSAFE=") compileOptions
    -- shouldNotBe threadsafeOption ("THREADSAFE=0" :: Text)
    shouldNotContain compileOptions ["THREADSAFE=0" :: Text]
