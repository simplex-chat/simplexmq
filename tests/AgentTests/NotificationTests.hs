...
testNotificationSubscriptionExistingConnection apns baseId alice@AgentClient {agentEnv = Env {config = aliceCfg, store}} bob = do
  ...
  [Nothing] <- getConnectionMessages alice [ConnMsgReq cId $ Just $ systemToUTCTime msgTs]
  ...
  withAgent 3 aliceCfg initAgentServers testDB $ \aliceNtf -> do
    (Just SMPMsgMeta {msgFlags = MsgFlags True}) :| _ <- getConnectionMessages aliceNtf [ConnMsgReq cId $ Just $ systemToUTCTime msgTs]
    pure ()
