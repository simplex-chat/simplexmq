  ...
  describe "JOIN" $ do
    describe "valid" $ do
      ...
      it "using same server as in invitation" $
        ("311", "a", "JOIN https://simpex.chat/invitation#/?smp=smp%3A%2F%2F" <> (U.encode . decodeLenient) "JGjNj5pWmlfrnMPxEwr6xjyEonjUOQDw1ymt3iwWOjs=" <> "%40localhost%3A5001%2F3456-w%3D%3D%23&e2e=" <> urlEncode True samplePublicKey <> " 14\nbob's connInfo")
          >#> ("311", "a", "ERR SMP AUTH")
    describe "invalid" $ do
