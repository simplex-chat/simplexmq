...
pattern Resp :: SMP.CorrId -> SMP.QueueId -> SMP.Command 'SMP.Broker -> SMP.TransmissionOrError
pattern Resp corrId queueId command <- ("", (corrId, queueId, Right (SMP.Cmd SMP.SBroker command)))

sendRecv :: Handle -> (ByteString, ByteString, ByteString, ByteString) -> IO SMP.TransmissionOrError
sendRecv h (sgn, corrId, qId, cmd) = SMP.tPutRaw h (sgn, corrId, encode qId, cmd) >> SMP.tGet SMP.fromServer h

(>#>) :: SMP.RawTransmission -> SMP.RawTransmission -> Expectation
...
testCreateSecure =
  it "should create (NEW) and secure (KEY) queue" $
    smpTest \h -> do
      Resp "abcd" rId1 (SMP.IDS rId sId) <- sendRecv h ("", "abcd", "", "NEW 1234")
      (rId1, "") #== "creates queue"

      Resp "bcda" sId1 ok1 <- sendRecv h ("", "bcda", sId, "SEND :hello")
      (ok1, SMP.OK) #== "accepts unsigned SEND"
      (sId1, sId) #== "same queue ID in response 1"

      Resp "" _ (SMP.MSG _ _ msg1) <- SMP.tGet SMP.fromServer h
      (msg1, "hello") #== "delivers message"

      Resp "cdab" _ ok4 <- sendRecv h ("1234", "cdab", rId, "ACK")
      (ok4, SMP.OK) #== "replies OK when message acknowledged if no more messages"

      Resp "dabc" _ err6 <- sendRecv h ("1234", "dabc", rId, "ACK")
      (err6, SMP.ERR SMP.PROHIBITED) #== "replies ERR when message acknowledged without messages"

      Resp "abcd" sId2 err1 <- sendRecv h ("4567", "abcd", sId, "SEND :hello")
      (err1, SMP.ERR SMP.AUTH) #== "rejects signed SEND"
      (sId2, sId) #== "same queue ID in response 2"

      Resp "bcda" _ err2 <- sendRecv h ("12345678", "bcda", rId, "KEY 4567")
      (err2, SMP.ERR SMP.AUTH) #== "rejects KEY with wrong signature (password atm)"

      Resp "cdab" _ err3 <- sendRecv h ("1234", "cdab", sId, "KEY 4567")
      (err3, SMP.ERR SMP.AUTH) #== "rejects KEY with sender's ID"

      Resp "dabc" rId2 ok2 <- sendRecv h ("1234", "dabc", rId, "KEY 4567")
      (ok2, SMP.OK) #== "secures queue"
      (rId2, rId) #== "same queue ID in response 3"

      Resp "abcd" _ err4 <- sendRecv h ("1234", "abcd", rId, "KEY 4567")
      (err4, SMP.ERR SMP.AUTH) #== "rejects KEY if already secured"

      Resp "bcda" _ ok3 <- sendRecv h ("4567", "bcda", sId, "SEND 11\nhello again")
      (ok3, SMP.OK) #== "accepts signed SEND"

      Resp "" _ (SMP.MSG _ _ msg) <- SMP.tGet SMP.fromServer h
      (msg, "hello again") #== "delivers message 2"

      Resp "cdab" _ ok5 <- sendRecv h ("1234", "cdab", rId, "ACK")
      (ok5, SMP.OK) #== "replies OK when message acknowledged 2"

      Resp "dabc" _ err5 <- sendRecv h ("", "dabc", sId, "SEND :hello")
      (err5, SMP.ERR SMP.AUTH) #== "rejects unsigned SEND"

testCreateDelete :: Spec
testCreateDelete =
  it "should create (NEW), suspend (OFF) and delete (DEL) queue" $
    smpTest2 \rh sh -> do
      Resp "abcd" rId1 (SMP.IDS rId sId) <- sendRecv rh ("", "abcd", "", "NEW 1234")
      (rId1, "") #== "creates queue"

      Resp "bcda" _ ok1 <- sendRecv rh ("1234", "bcda", rId, "KEY 4567")
      (ok1, SMP.OK) #== "secures queue"

      Resp "cdab" _ ok2 <- sendRecv sh ("4567", "cdab", sId, "SEND :hello")
      (ok2, SMP.OK) #== "accepts signed SEND"

      Resp "dabc" _ ok7 <- sendRecv sh ("4567", "dabc", sId, "SEND :hello 2")
      (ok7, SMP.OK) #== "accepts signed SEND 2 - this message is not delivered because the first is not ACKed"

      Resp "" _ (SMP.MSG _ _ msg1) <- SMP.tGet SMP.fromServer rh
      (msg1, "hello") #== "delivers message"

      Resp "abcd" _ err1 <- sendRecv rh ("12345678", "abcd", rId, "OFF")
      (err1, SMP.ERR SMP.AUTH) #== "rejects OFF with wrong signature (password atm)"

      Resp "bcda" _ err2 <- sendRecv rh ("1234", "bcda", sId, "OFF")
      (err2, SMP.ERR SMP.AUTH) #== "rejects OFF with sender's ID"

      Resp "cdab" rId2 ok3 <- sendRecv rh ("1234", "cdab", rId, "OFF")
      (ok3, SMP.OK) #== "suspends queue"
      (rId2, rId) #== "same queue ID in response 2"

      Resp "dabc" _ err3 <- sendRecv sh ("4567", "dabc", sId, "SEND :hello")
      (err3, SMP.ERR SMP.AUTH) #== "rejects signed SEND"

      Resp "abcd" _ err4 <- sendRecv sh ("", "abcd", sId, "SEND :hello")
      (err4, SMP.ERR SMP.AUTH) #== "reject unsigned SEND too"

      Resp "bcda" _ ok4 <- sendRecv rh ("1234", "bcda", rId, "OFF")
      (ok4, SMP.OK) #== "accepts OFF when suspended"

      Resp "cdab" _ (SMP.MSG _ _ msg) <- sendRecv rh ("1234", "cdab", rId, "SUB")
      ...
      (err5, SMP.ERR SMP.AUTH) #== "rejects DEL with wrong signature (password atm)"

      Resp "abcd" _ err6 <- sendRecv rh ("1234", "abcd", sId, "DEL")
      (err6, SMP.ERR SMP.AUTH) #== "rejects DEL with sender's ID"

      Resp "bcda" rId3 ok6 <- sendRecv rh ("1234", "bcda", rId, "DEL")
      (ok6, SMP.OK) #== "deletes queue"
      (rId3, rId) #== "same queue ID in response 3"

      Resp "cdab" _ err7 <- sendRecv sh ("4567", "cdab", sId, "SEND :hello")
      (err7, SMP.ERR SMP.AUTH) #== "rejects signed SEND when deleted"

      Resp "dabc" _ err8 <- sendRecv sh ("", "dabc", sId, "SEND :hello")
      (err8, SMP.ERR SMP.AUTH) #== "rejects unsigned SEND too when deleted"

      Resp "abcd" _ err11 <- sendRecv rh ("1234", "abcd", rId, "ACK")
      (err11, SMP.ERR SMP.AUTH) #== "rejects ACK when conn deleted - the second message is deleted"

      Resp "bcda" _ err9 <- sendRecv rh ("1234", "bcda", rId, "OFF")
      (err9, SMP.ERR SMP.AUTH) #== "rejects OFF when deleted"

      Resp "cdab" _ err10 <- sendRecv rh ("1234", "cdab", rId, "SUB")
      (err10, SMP.ERR SMP.AUTH) #== "rejects SUB when deleted"

testDuplex :: Spec
testDuplex =
  it "should create 2 simplex connections and exchange messages" $
    smpTest2 \alice bob -> do
      Resp "abcd" _ (SMP.IDS aRcv aSnd) <- sendRecv alice ("", "abcd", "", "NEW 1234")
      -- aSnd ID is passed to Bob out-of-band

      Resp "bcda" _ SMP.OK <- sendRecv bob ("", "bcda", aSnd, "SEND :key efgh")
      -- "key efgh" is ad-hoc, different from SMP protocol

      Resp "" _ (SMP.MSG _ _ msg1) <- SMP.tGet SMP.fromServer alice
      Resp "cdab" _ SMP.OK <- sendRecv alice ("1234", "cdab", aRcv, "ACK")
      ["key", key1] <- return $ B.words msg1
      (key1, "efgh") #== "key received from Bob"
      Resp "dabc" _ SMP.OK <- sendRecv alice ("1234", "dabc", aRcv, "KEY " <> key1)

      Resp "abcd" _ (SMP.IDS bRcv bSnd) <- sendRecv bob ("", "abcd", "", "NEW abcd")
      Resp "bcda" _ SMP.OK <- sendRecv bob ("efgh", "bcda", aSnd, "SEND :reply_id " <> encode bSnd)
      -- "reply_id ..." is ad-hoc, it is not a part of SMP protocol

      Resp "" _ (SMP.MSG _ _ msg2) <- SMP.tGet SMP.fromServer alice
      Resp "cdab" _ SMP.OK <- sendRecv alice ("1234", "cdab", aRcv, "ACK")
      ["reply_id", bId] <- return $ B.words msg2
      (bId, encode bSnd) #== "reply queue ID received from Bob"
      Resp "dabc" _ SMP.OK <- sendRecv alice ("", "dabc", bSnd, "SEND :key 5678")
      -- "key 5678" is ad-hoc, different from SMP protocol

      Resp "" _ (SMP.MSG _ _ msg3) <- SMP.tGet SMP.fromServer bob
      Resp "abcd" _ SMP.OK <- sendRecv bob ("abcd", "abcd", bRcv, "ACK")
      ["key", key2] <- return $ B.words msg3
      (key2, "5678") #== "key received from Alice"
      Resp "bcda" _ SMP.OK <- sendRecv bob ("abcd", "bcda", bRcv, "KEY " <> key2)

      Resp "cdab" _ SMP.OK <- sendRecv bob ("efgh", "cdab", aSnd, "SEND :hi alice")

      Resp "" _ (SMP.MSG _ _ msg4) <- SMP.tGet SMP.fromServer alice
      Resp "dabc" _ SMP.OK <- sendRecv alice ("1234", "dabc", aRcv, "ACK")
      (msg4, "hi alice") #== "message received from Bob"

      Resp "abcd" _ SMP.OK <- sendRecv alice ("5678", "abcd", bSnd, "SEND :how are you bob")

      Resp "" _ (SMP.MSG _ _ msg5) <- SMP.tGet SMP.fromServer bob
      Resp "bcda" _ SMP.OK <- sendRecv bob ("abcd", "bcda", bRcv, "ACK")
...
testSwitchSub =
  it "should create simplex connections and switch subscription to another TCP connection" $
    smpTest3 \rh1 rh2 sh -> do
      Resp "abcd" _ (SMP.IDS rId sId) <- sendRecv rh1 ("", "abcd", "", "NEW 1234")
      Resp "bcda" _ ok1 <- sendRecv sh ("", "bcda", sId, "SEND :test1")
      (ok1, SMP.OK) #== "sent test message 1"
      Resp "cdab" _ ok2 <- sendRecv sh ("", "cdab", sId, "SEND :test2, no ACK")
      (ok2, SMP.OK) #== "sent test message 2"

      Resp "" _ (SMP.MSG _ _ msg1) <- SMP.tGet SMP.fromServer rh1
      (msg1, "test1") #== "test message 1 delivered to the 1st TCP connection"
      Resp "abcd" _ (SMP.MSG _ _ msg2) <- sendRecv rh1 ("1234", "abcd", rId, "ACK")
      (msg2, "test2, no ACK") #== "test message 2 delivered, no ACK"

      Resp "bcda" _ (SMP.MSG _ _ msg2') <- sendRecv rh2 ("1234", "bcda", rId, "SUB")
      (msg2', "test2, no ACK") #== "same simplex queue via another TCP connection, tes2 delivered again (no ACK in 1st queue)"
      Resp "cdab" _ SMP.OK <- sendRecv rh2 ("1234", "cdab", rId, "ACK")

      Resp "" _ end <- SMP.tGet SMP.fromServer rh1
      (end, SMP.END) #== "unsubscribed the 1st TCP connection"

      Resp "dabc" _ SMP.OK <- sendRecv sh ("", "dabc", sId, "SEND :test3")

      Resp "" _ (SMP.MSG _ _ msg3) <- SMP.tGet SMP.fromServer rh2
      (msg3, "test3") #== "delivered to the 2nd TCP connection"

      Resp "abcd" _ err <- sendRecv rh1 ("1234", "abcd", rId, "ACK")
      (err, SMP.ERR SMP.PROHIBITED) #== "rejects ACK from the 1st TCP connection"

      Resp "bcda" _ ok3 <- sendRecv rh2 ("1234", "bcda", rId, "ACK")
      (ok3, SMP.OK) #== "accepts ACK from the 2nd TCP connection"

      timeout 1000 (SMP.tGet SMP.fromServer rh1) >>= \case
        Nothing -> return ()
