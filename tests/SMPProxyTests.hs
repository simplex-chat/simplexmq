...
deliverMessagesViaProxy proxyServ relayServ alg unsecuredMsgs securedMsgs = do
  ...
  -- set up proxy
  ts <- getCurrentTime
  pc' <- getProtocolClient 0 g (1, proxyServ, Nothing) defaultSMPClientConfig {serverVRange = mkVersionRange minServerSMPRelayVersion currentClientSMPRelayVersion} [] Nothing ts (\_ -> pure ())
  ...
  rc' <- getProtocolClient 0 g (2, relayServ, Nothing) defaultSMPClientConfig {serverVRange = mkVersionRange minServerSMPRelayVersion authCmdsSMPVersion} [] (Just msgQ) ts (\_ -> pure ())
...
proxyConnectDeadRelay n d proxyServ = do
  ...
  -- set up proxy
  ts <- getCurrentTime
  pc' <- getProtocolClient 0 g (1, proxyServ, Nothing) defaultSMPClientConfig {serverVRange = mkVersionRange minServerSMPRelayVersion sendingProxySMPVersion} [] Nothing ts (\_ -> pure ())
  pc <- either (fail . show) pure pc'
  THAuthClient {} <- maybe (fail "getProtocolClient returned no thAuth") pure $ thAuth $ thParams pc
  -- get proxy session
  replicateM_ n $ do
