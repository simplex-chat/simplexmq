...
7. In case of successful decryption and matching key, the server should send encrypted welcome header.

```abnf
aes_welcome_header = aes_header_auth_tag aes_encrypted_header
welcome_header = smp_version ["," smp_mode] *SP ; 32 bytes
smp_version = %s"v" 1*DIGIT "." 1*DIGIT "." 1*DIGIT ["-" 1*ALPHA "." 1*DIGIT] ; in semver format
  ; for example: v123.456.789-alpha.7
smp_mode = smp_public / smp_private
smp_public = %s"pub" ; public (default) - no auth to create and manage queues
smp_private = %s"auth" ; server authentication with AUTH command (TBD) is required to create and manage queues
...
header = padded_body_size payload_size reserved ; 32 bytes
...
bytes16 = 16*16(OCTET) ; with 256 bit AES key size
