...
getQueues :: (MsgStoreClass s, QueueParty p) => s -> SParty p -> [QueueId] -> IO [Either ErrorType (StoreQueue s)]
...
getQueueRecs :: (MsgStoreClass s, QueueParty p) => s -> SParty p -> [QueueId] -> IO [Either ErrorType (StoreQueue s, QueueRec)]
