...
serverSchemaMigrations =
  [ ("20250207_initial", m20250207_initial, Nothing),
    ...
    ("20260918_expire_messages", m20260918_expire_messages, Just down_m20260918_expire_messages)
  ]
...
m20260918_expire_messages :: Text
m20260918_expire_messages =
  [r|
CREATE INDEX idx_messages_expire ON messages (msg_ts, recipient_id) WHERE NOT msg_quota;
...
CREATE OR REPLACE FUNCTION delete_expired_msgs(p_recipient_id bytea, p_old_ts bigint) RETURNS bigint
    LANGUAGE plpgsql
    AS $$
DECLARE
  q_size BIGINT;
...
CREATE OR REPLACE FUNCTION write_message(p_recipient_id bytea, p_msg_id bytea, p_msg_ts bigint, p_msg_quota boolean, p_msg_ntf_flag boolean, p_msg_body bytea, p_quota integer) RETURNS TABLE(quota_written boolean, was_empty boolean)
    LANGUAGE plpgsql
    AS $$
DECLARE
  q_can_write BOOLEAN;
...
CREATE OR REPLACE FUNCTION try_del_msg(p_recipient_id bytea, p_msg_id bytea) RETURNS TABLE(r_msg_id bytea, r_msg_ts bigint, r_msg_quota boolean, r_msg_ntf_flag boolean, r_msg_body bytea)
    LANGUAGE plpgsql
    AS $$
DECLARE
  q_size BIGINT;
...
CREATE OR REPLACE FUNCTION try_del_peek_msg(p_recipient_id bytea, p_msg_id bytea) RETURNS TABLE(r_msg_id bytea, r_msg_ts bigint, r_msg_quota boolean, r_msg_ntf_flag boolean, r_msg_body bytea)
    LANGUAGE plpgsql
    AS $$
DECLARE
  q_size BIGINT;
...
ALTER TABLE msg_queues DROP COLUMN msg_queue_expire;
    |]

down_m20260918_expire_messages :: Text
down_m20260918_expire_messages =
  [r|
...
DROP INDEX idx_messages_expire;
