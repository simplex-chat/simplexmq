  ...
  where
    ...
    getRealTimeMetrics Env {sockets, msgStore = AMS _ _ ms, server = srv@Server {subscribers, ntfSubscribers}} = do
      ...
      where
        ...
        getDeliveredMetrics (RoundedSystemTime ts') = foldM countClnt (RTSubscriberMetrics 0 0, TimeAggregations 0 0 IM.empty) =<< getServerClients srv
          where
            countClnt acc@(metrics, times) (AClient _ _ Client {subscriptions}) = do
        ...
        getMessage q qr = time "GET" $ do
          ...
          where
            ...
            newSub = do
              ...
              pure s
            getMessage_ :: Sub -> Maybe (MsgId, RoundedSystemTime) -> M (Transmission BrokerMsg)
