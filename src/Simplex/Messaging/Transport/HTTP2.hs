...
import Simplex.Messaging.Transport (ALPN, SessionId, TLS (tlsUniq), Transport (cGet, cPut), tlsALPN)
...
withHTTP2 :: BufferSize -> (Config -> SessionId -> Maybe ALPN -> IO a) -> TLS -> IO a
withHTTP2 sz run c = E.bracket (allocHTTP2Config c sz) freeSimpleConfig $ \cfg -> run cfg (tlsUniq c) (tlsALPN c)
