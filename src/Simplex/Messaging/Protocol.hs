...
data BrokerMsg where
  ...
  PKEY :: TransportHost -> SessionId -> VersionRangeSMP -> (X.CertificateChain, X.SignedExact X.PubKey) -> BrokerMsg -- TLS-signed server key for proxy shared secret and initial sender key
...
instance ProtocolEncoding SMPVersion ErrorType BrokerMsg where
  ...
  encodeProtocol _v = \case
    ...
    PKEY host sid vr (cert, key) -> e (PKEY_, ' ', host, sid, vr, C.encodeCertChain cert, C.SignedObject key)
  ...
  protocolP _v = \case
    ...
    PKEY_ -> PKEY <$> _smpP <*> smpP <*> smpP <*> ((,) <$> C.certChainP <*> (C.getSignedExact <$> smpP))
