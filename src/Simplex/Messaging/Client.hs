...
okSMPCommand :: Cmd -> SMPClient -> C.PrivateKey t -> QueueId -> ExceptT SMPClientError IO ()
okSMPCommand cmd c pKey qId =
  sendSMPCommand c (Just pKey) qId cmd >>= \case
    Cmd _ OK -> return ()
    _ -> throwE SMPUnexpectedResponse

sendSMPCommand :: SMPClient -> Maybe (C.PrivateKey t) -> QueueId -> Cmd -> ExceptT SMPClientError IO Cmd
