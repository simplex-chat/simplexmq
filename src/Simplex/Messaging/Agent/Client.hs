  ...
  liftIO $ do
    ...
    getProtocolClient g tSess cfg Nothing (\_ -> pure ()) >>= \case
      Right smp -> do
        ...
        r <- runExceptT $ do
          SMP.QIK {rcvId} <- liftError (testErr TSCreateQueue) $ createSMPQueue smp rKeys dhKey auth SMSubscribe Nothing
...
newRcvQueue c userId connId (ProtoServerWithAuth srv auth) vRange subMode = do
  ...
  (sessId, QIK {rcvId, sndId, rcvPublicDhKey}) <-
    withClient c tSess "NEW" $ \(SMPConnectedClient smp _) ->
      (sessionId $ thParams smp,) <$> createSMPQueue smp rKeys dhKey auth subMode Nothing
