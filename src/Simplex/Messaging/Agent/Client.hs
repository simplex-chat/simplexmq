...
import Data.Functor (($>))
import Data.List (deleteFirstsBy, foldl', intersperse, partition, (\\))
...
newRcvQueue c userId connId (ProtoServerWithAuth srv auth) vRange subMode = do
  ...
  logServer "<--" c srv "" ["IDS", logSecret rcvId, logSecret sndId]
  let rq =
        RcvQueue
          { userId,
            connId,
...
logServer dir AgentClient {clientId} srv qId cmdStr =
  logInfo . decodeUtf8 $
    B.concat $
      "A (" : bshow clientId : ") " : dir : " " : showServer srv : " : " : logSecret qId : " " : intersperse " " cmdStr
...
agentXFTPNewChunk c SndFileChunk {userId, chunkSpec = XFTPChunkSpec {chunkSize}, digest = FileDigest chunkDigest} n (ProtoServerWithAuth srv auth) = do
  ...
  logServer "<--" c srv "" ["SIDS", logSecret sndId]
