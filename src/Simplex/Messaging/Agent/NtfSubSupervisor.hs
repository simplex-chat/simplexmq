  ...
  where
    ...
    createSubs ntfSubs =
      ...
      where
        ...
        splitSubs :: [(NtfSubscription, Either AgentErrorType RcvQueue)] -> ([(ConnId, AgentErrorType)], [CreateSubNtfReq])
    ...
    checkSubs ntfSubs =
      getNtfToken >>= \case
        ...
        _ -> do
          ...
          pure []
      where
        splitSubs :: [NtfSubscription] -> ([(ConnId, AgentErrorType)], [CheckSubNtfReq])
        splitSubs = foldr splitSub ([], [])
          where
