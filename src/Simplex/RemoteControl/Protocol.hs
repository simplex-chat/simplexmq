...
module Simplex.RemoteControl.Protocol where
...
-- import Simplex.RemoteControl.Controller
...
data RemoteResponse
  ...
  | RRResponse {chatResponse :: J.Value}
  | RREvent {chatEvent :: Maybe J.Value} -- ^ 'Nothing' on poll timeout
...
createRemoteHostClient :: HTTP2Client -> (C.APublicDhKey, C.APrivateDhKey) -> Text -> ExceptT RemoteProtocolError IO RemoteHostClient
createRemoteHostClient httpClient (dhPub, dhPriv) desktopName = do
